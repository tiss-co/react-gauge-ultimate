...
//Constants
const startAngle = -Math.PI / 1.35; //Negative x-axis
const endAngle = Math.PI / 1.35; //Positive x-axis
...
export const Gauge = props => {
  ...
  useDeepCompareEffect(() => {
    if (
      props.nrOfLevels ||
      prevProps.current.arcsLength.every(a =>
        props.arcsLength.includes(a)
      ) ||
  ...
  const { id, style, className, darkMode } = props;
  return (
    <div
      id={id}
      className={className}
...
Gauge.defaultProps = {
  ...
  nrOfLevels: 4,
  percent: 0,
  arcPadding: 0.01, //The padding between arcs, in rad
  arcWidth: 0.23, //The width of the arc given in percent of the radius
  colors: ['#4C586B', '#318C35', '#DD7128', '#952222'], //Default defined colors
  // textColor: '#000',
  // needleColor: '#464A4F',
...
const renderChart = (
  ...
  arcChart.current
    ...
    .cornerRadius(radius => {
      // if (radius.index === 0 || radius.index === props.colors.length - 1)
      //    return 25;
...
const calculateRotation = (percent, outerRadius, width) => {
  var needleLength = outerRadius.current * 0.55, //TODO: Maybe it should be specified as a percentage of the arc radius?
    ...
    centerPoint = [0, -needleRadius / 2],
    topPoint = [
      centerPoint[0] - needleLength * Math.cos(theta) * 1.25,
      centerPoint[1] - needleLength * Math.sin(theta) * 1.25,
    ],
    leftPoint = [
      centerPoint[0] - needleRadius * Math.cos(theta - Math.PI / 1.35),
      centerPoint[1] - needleRadius * Math.sin(theta - Math.PI / 1.35),
    ],
    rightPoint = [
      centerPoint[0] - needleRadius * Math.cos(theta + Math.PI / 1.35),
      centerPoint[1] - needleRadius * Math.sin(theta + Math.PI / 1.35),
...
const addText = (percentage, props, outerRadius, width, g) => {
  const { formatTextValue, fontSize, darkMode } = props;
  ...
  g.current
    ...
    .style('font-size', () =>
      fontSize
        ? fontSize
        : `${width.current / 7 / (text.length > 10 ? text.length / 10 : 1)
        }px`
    )
  ...
  g.current
    ...
    .attr(
      'transform',
      `translate(${outerRadius.current}, ${outerRadius.current / 2 + (textPadding - textPadding / 3)
      })`
    )
    .append('text')
    .text(props.label)
    // this computation avoid text overflow. When for matted value is over 10 characters, we should reduce font size
    .style('font-size', () =>
      fontSize
        ? fontSize / 2
        : `${width.current /
        10 /
        (text.length > 10 ? text.length / 10 : 1) /
        2
        }px`
    )
    // .style('fill', props.textColor)
...
const updateDimensions = (props, container, margin, width, height) => {
  ...
  const { marginInPercent, id } = props;
  let divDimensions = container.current.node().getBoundingClientRect(),
    divWidth = divDimensions.width,
    divHeight = divDimensions.height;

  if (id) {
    divDimensions = document?.getElementById(id);
    divWidth = divDimensions?.clientWidth || divWidth;
    divHeight = divDimensions?.clientHeight || divHeight;
  }
